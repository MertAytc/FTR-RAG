    ...
    for page in pages:
        ...
        for block in blocks:
            ...
            if block_type == "text":
                ...
                for piece in pieces:
                    # A functioning-domain split is a deliberate, known-good
                    # boundary (one ICF domain's assessment/intervention
                    # rows), so it is exempt from the general short-chunk
                    # filter -- some domains legitimately have only two or
                    # three short lines of content, and dropping them would
                    # silently lose real clinical content.
                    if len(piece) < MIN_TEXT_CHARS and not block_functioning_domain:
                        continue
                ...
                if block_local_heading:
                    metadata["local_heading"] = block_local_heading
                all_chunks.append({"text": block_text, "metadata": metadata})
        ...
        if chunk["metadata"].get("source_type") == "text":
            ...
            if tokens < ORPHAN_TOKEN_THRESHOLD:
                orphan_candidates.append({
                    ...
                    "local_heading": chunk["metadata"].get("local_heading"),
                })
