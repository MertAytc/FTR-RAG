...
from src.retrieval.vector_store import get_embeddings_by_ids
...
class MMRRetriever:
    ...
    def _prepare_candidates(
        ...
        query_embedding = embed_texts([semantic_query], input_type="query")[0]
        # IMPORTANT: candidate chunks are already embedded in the active
        # Chroma index. Reuse those stored vectors instead of sending all
        # candidate documents through the embedding model again. This keeps
        # MMR consistent with the active embedding index and avoids a very
        # expensive re-embedding pass for every query.
        candidate_ids = [str(item["id"]) for item in candidates]
        stored_embeddings = get_embeddings_by_ids(candidate_ids)

        missing_ids = [
            chunk_id for chunk_id in candidate_ids
            if chunk_id not in stored_embeddings
        ]

        if missing_ids:
            raise RuntimeError(
                "MMR için bazı aday chunk embeddingleri Chroma indexinde "
                f"bulunamadı: {missing_ids[:5]}"
            )

        candidate_embeddings = [stored_embeddings[chunk_id] for chunk_id in candidate_ids]
