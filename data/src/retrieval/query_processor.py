...
def _phrase_matches(query: str, term: str) -> bool:
    ...
    # Exact phrase match must respect token boundaries. A raw substring check
    # would incorrectly match short glossary terms such as "el" inside "bel".
    if t:
        escaped = re.escape(t)
...
def rewrite_query_to_english(query: str, matched_terms: List[Tuple[str, List[str]]]) -> str:
    """Rewrite a Turkish FTR query into concise clinical English for retrieval.

    This is a semantic rewrite, not a literal translation. The model is instructed
    to preserve clinical meaning, population, condition, intervention, outcome,
    progression/dosage/safety constraints and any explicit numbers.
    """
    if ollama is None:
...
"""
    ...
    response = ollama.chat(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a clinical terminology-aware query rewriting assistant. Rewrite only; never answer or add facts."
            },
    ...
    # Deterministic guard: if a validated clinical term from the user's
    # Turkish query disappeared from the rewrite, restore one English
    # equivalent. This prevents losses such as "inme" -> generic "injury".
...
def _contains_term(text: str, term: str) -> bool:
    """Check whether an English terminology hint occurs as a whole phrase."""
    q = normalize(text)
    t = normalize(term)
    if not t:
        return False
...
def _preserve_terminology_anchors(
    ...
    """
    Prevent the LLM rewrite from dropping clinically important dictionary terms.

    The rewrite model is still responsible for phrasing the query. This helper
    only restores a validated English equivalent when the model omitted every
    equivalent supplied by the terminology dictionary.
    """
    anchors = []
    for _, equivalents in matched_terms:
...
def process_query(
    ...
    # BM25 gets the English clinical rewrite plus the validated terminology
    # expansion. The original Turkish query is retained for traceability.
...
if __name__ == "__main__":
    query = input("Türkçe sorgu: ").strip()
    result = process_query(query)
    print("\nEşleşen FTR terimleri:")
    for term in result["matched_terms"]:
        print(f"- {term}")
    print("\nEnglish clinical rewrite:")
